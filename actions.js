...
function registerActions(bot) {
  ...
  bot.action("info", async (ctx) => {
    ...
    const [user, subs] = await Promise.all([
      prisma.user.findUnique({ where: { id: ctx.dbUser.id } }),
    ...
    const subsText =
      subs.length === 0
        ...
        : subs
            .map((s, i) => {
              const label = s.type === "FREE" ? "Free" : PLANS[s.type]?.label || s.type;
              return `${i + 1}) ${label} — до ${formatDate(s.endDate)}`;
            })
            .join("\n");

    const text = `📋 Информация\n\nБаланс: ${ruMoney(user.balance)}\n\n${subsText}`;
    await editOrAnswer(ctx, text, mainMenu(user.balance));
  });
