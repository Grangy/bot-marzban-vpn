...
Нажмите кнопку ниже, чтобы перейти в магазин приложений.`;
  ...
  bot.action(/^setup_downloaded_(ios|android|android_tv|windows|macos)_(\d+)$/, async (ctx) => {
    ...
    // Формируем краткий текст инструкции (без ссылок)
    const getInstructionText = (deviceType) => {
      // Специальная инструкция для Android TV
      if (deviceType === 'android_tv') {
        return `📺 Пошаговая настройка для Android TV:
...
🌐 Способ 2: Через сайт tv.happ.su
На телевизоре выберите пункт «Web Import» и введите отображённый код либо отсканируйте QR-код и откройте ссылку в браузере телефона.

Подключитесь по кнопкам ниже.`;
      }
      
      const deviceNames = {
        ios: { name: "iPhone" },
        android: { name: "вашем устройстве" },
        windows: { name: "вашем компьютере" },
        macos: { name: "вашем Mac" }
      };
      
      const device = deviceNames[deviceType];
      
      let text = `📱 Пошаговая настройка:\n\n`;
      text += `1) Откройте приложение Happ на ${device.name}\n\n`;
      text += `2) Нажмите кнопку "+" в правом верхнем углу\n\n`;
      text += `3) Подключитесь по кнопкам ниже\n\n`;
      text += `4) Выберите "вставить из буфера обмена"\n\n`;
      text += `5) Нажмите кнопку "разрешить вставку"\n\n`;
      text += `6) Нажмите "Import"\n\n`;
      text += `7) После импорта нажмите на созданную конфигурацию\n\n`;
    ...
    const fullMessage = getInstructionText(device);

    // Кнопки для подключения (с URL ссылками)
    const buttons = [];
