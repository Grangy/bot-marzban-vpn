...
/** Нормализованные ID админ-чатов: и короткий (-X), и полный (-100X) */
function getAdminChatIdVariants() {
  const set = new Set();
  for (const cfg of ADMIN_GROUP_IDS) {
    set.add(String(cfg));
    const num = cfg.replace(/^-/, "");
    if (cfg.startsWith("-100")) {
      set.add(`-${num.replace(/^100/, "")}`);
    } else {
      set.add(`-100${num}`);
    }
  }
...
const ADMIN_CHAT_ID_VARIANTS = getAdminChatIdVariants();

/** Проверка: чат — админская группа. Поддержка -3872960505 и -1003872960505 (оба формата). */
function isAdminGroup(chatId) {
  if (!chatId) return false;
  const id = String(chatId);
  return ADMIN_CHAT_ID_VARIANTS.has(id);
}
...
📋 Order ID: <code>${topup.orderId}</code>`;
  ...
  // В админ-группу отправляем только успешные пополнения.

  // Запуск ежедневной статистики в 20:00
  scheduleDaily(20, 0, () => sendStats());

  console.log("📢 Admin notifier initialized. ADMIN_GROUP_ID=" + ADMIN_GROUP_IDS.join(",") + " → принимаются оба формата: " + [...ADMIN_CHAT_ID_VARIANTS].join(", "));
